...
# ///
"""Generate the September-start portion of the summer plan."""
...
def main() -> None:
    configure()
    rows = [row for row in read_csv(RAW / "summer_plan.csv") if int(row["week"]) >= 6]
    ...
    ax.set_xlim(date(2026, 9, 7), date(2026, 10, 1))
    ...
    save(fig, "summer_plan_september")
